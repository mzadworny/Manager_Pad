...
Many tools can approximate this workflow if you set them up yourself (e.g. Notion), but few offer a specific, purpose-built solution for the manager 1-on-1 workflow. General-purpose note apps grow to fulfill many applications, shifting the setup burden onto the manager. At thousands of users, recall and summaries must work with years of meeting history per employee; AI summary costs may require tiering or limits.
...
1. **What are the target `qps` and `data_volume` ballparks for `target_scale`?** — Owner: user. Currently `null` in frontmatter.
2. **Primary flow step 3 vs FR-003:** Primary success criteria mentions assigning employees to multiple teams; FR-003 resolves single team per employee for v1. Which wording is authoritative for MVP? — Owner: user.
3. **FR-011 privacy-preserving AI approach:** If AI summaries ship in v1, what privacy model is acceptable (e.g. explicit consent before processing)? — Owner: user. Block: no for MVP (nice-to-have), yes before implementation.
