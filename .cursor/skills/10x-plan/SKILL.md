...
2. **Spawn initial research tasks to gather context** (skip or narrow based on Step 1.0):
   Before asking the user any questions, create parallel sub-tasks to research:
   - **Explore agent** (a sub-agent focused on exploration) — find all files related to the task, search for patterns, trace code paths. Use for file discovery and codebase structure questions.
   - **general-purpose agent** (a general-purpose sub-agent) — for deeper analysis that may require reading many files and synthesizing findings. Use for understanding complex systems.
...
6. **Ask deep probing questions**:
   ...
   **Rules for structuring questions:**
   ...
   - Allow multiple selections only when choices aren't mutually exclusive
   - Keep the question header short (max 12 chars): "Scope", "Edge cases", "Priority"
...
4. **Spawn parallel sub-tasks for comprehensive research**:
   ...
   **For codebase investigation:**
   - **Explore** (a sub-agent focused on exploration) — Fast file/pattern search, code structure analysis
   - **general-purpose** (a general-purpose sub-agent) — Deep analysis requiring multi-step reasoning
