...
1. **Resolve the change folder, then write the plan** to `context/changes/<change-id>/plan.md`.
   ...
   - **Linear sync (best effort):** if `change.md.linear_issue` is a non-null issue id (e.g. `MAC-5`), and the Linear MCP is available, update that issue: set state `Todo` (or keep `In Progress` if already started), and add a comment `Plan written → context/changes/<change-id>/plan.md (status: planned)`. If MCP is unavailable or `linear_issue` is null, skip silently.
