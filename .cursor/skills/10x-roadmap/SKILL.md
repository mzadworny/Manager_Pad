...
This table is the clean handoff to Jira/Linear or any MCP-backed backlog. Include one row for every `F-NN` and `S-NN`. It should be compact enough to copy into issues, but it must not duplicate the detailed roadmap body. After issues are created, fill the **Linear** column with the issue id (or a markdown link); `/10x-new` copies it into `change.md.linear_issue` for agent sync.
